...
class ExponentialNLL:
    def __init__(self, eps: float=1e-6) -> None:
        self.eps = eps

    def __call__(self, output, target):
        return torch.mean((output + self.eps)*target - torch.log(output + self.eps))
...
def train(
    ...
    if config["mode"] == "regression":   
        ...
        raise ValueError(f"Expected mode to be regression or classification, got {config['mode']}.")

    model = ForseaAutoEncoder(
        dataset.ocean_data_shape,
    ...
    elif config["criterion"] == "exponential_nll":
        criterion = ExponentialNLL()
    else:
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        for epoch in range(config["epochs"]):
            ...
            for batch_idx, data in enumerate(dataset.train_data()):
                ...
                loss.backward()
                if ((batch_idx+1) % batch_accumulation == 0) or (batch_idx == dataset.train_n-1):
                    optimizer.step()
                    optimizer.zero_grad()
                ...
                pred = outputs > 0.5
                train_accuracy += torch.sum(pred == target) / len(config["fish_types"])
                # print metrics
                if (train_batches+1) % log_period == 0:
                    print(
                        f"[{epoch + 1:3d}, {train_batches+1:5d}] | loss: {running_loss / train_samples:.6f} accuracy: {train_accuracy / train_samples:.6f}"
                    )
                    mlflow.log_metric(
                        "train_loss", running_loss / train_samples, step=log_step
                    )
                    mlflow.log_metric(
                        "train_accuracy", train_accuracy / train_samples, step=log_step
                    )
                    log_step += 1
                    running_loss = 0.0
                    train_samples = 0
            ...
            with torch.no_grad():
                for _, data in enumerate(dataset.val_data()):
                    ...
                    pred = outputs > 0.5
                    val_accuracy += torch.sum(pred == target) / len(config["fish_types"])
                    val_samples += len(route_input)

            print(
                f"[{epoch + 1:3d}, {train_samples:5d}] | train loss: {running_loss / train_samples:.6f} val accuracy: {train_accuracy / train_samples:.6f} "
                f"| val loss: {val_loss / val_samples:.6f} val accuracy: {val_accuracy / val_samples:.6f} "
            )
            mlflow.log_metric("val_loss", val_loss / val_samples, step=log_step)
            mlflow.log_metric("val_accuracy", val_accuracy / val_samples, step=log_step)
    return model
...
if __name__ == "__main__":
    ...
    train(config, run_name="Gaussian_filter_log_target")
